    ...
    return (
        <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-0 my-6 py-4 bg-[color-mix(in_srgb,var(--np-ink)_3%,transparent)] border border-[var(--np-rule)]">
                {SKILLS.map((category) => (
                    <div key={category.id} className="px-4 text-center relative border-r border-dotted border-[var(--np-rule)] md:[&:nth-child(3n)]:border-r-0 lg:[&:nth-child(3n)]:border-r lg:[&:nth-child(5n)]:border-r-0 [&:nth-child(2n)]:border-r-0 md:[&:nth-child(2n)]:border-r">
